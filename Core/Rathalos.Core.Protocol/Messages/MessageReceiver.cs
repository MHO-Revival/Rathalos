...
namespace Rathalos.Core.Protocol.Messages
{
	public static class MessageReceiver
	{
        ...
        private static readonly Dictionary<uint, (Type Type, Func<object> Constructor)> _messagesDefinitions = new Dictionary<uint, (Type Type, Func<object> Constructor)>();
		...
		public static void Initialize()
		{
			...
			IEnumerable<Type> messageTypes = asm.GetTypes().Where(x => x.IsSubclassOf(typeof(Message)) && !x.IsAbstract);
			foreach (Type type in messageTypes)
			{
				var fieldId = type.GetField(nameof(Message.ProtocolMessageId));
