// <auto-generated />
// This file was generated from XML metalib definition
// Do not modify this file manually as it will be overwritten
using System;
using System.Collections.Generic;
using Rathalos.Core.Utils.IO;

namespace Rathalos.Core.Protocol.Messages.Csproto
{
    /// <summary>
    /// Set avatar tag
    /// </summary>
    public class CSAvatarSetNtf : CSPkgBody
    {
		public const CS_CMD_ID ProtocolIdConst = CS_CMD_ID.CS_CMD_AVATAR_SET_NTF;
		/// <summary>
		/// Protocol identifier for this message type
		/// </summary>
		public ushort ProtocolId => (ushort)ProtocolIdConst;
		/// <summary>
		/// player
		/// </summary>
		public uint RoleId { get; set; }
		/// <summary>
		/// Hide fashion
		/// </summary>
		public byte HideFashion { get; set; }
		/// <summary>
		/// Hide suite
		/// </summary>
		public byte HideSuite { get; set; }
		/// <summary>
		/// Hide helmet
		/// </summary>
		public byte HideHelm { get; set; }

		/// <summary>
		/// Serializes this instance to the specified data writer.
		/// </summary>
		/// <param name="writer">The data writer to serialize to.</param>
		public void Serialize(IDataWriter writer)
		{
			writer.WriteUInt(RoleId);
			writer.WriteByte(HideFashion);
			writer.WriteByte(HideSuite);
			writer.WriteByte(HideHelm);
		}

		/// <summary>
		/// Deserializes this instance from the specified data reader.
		/// </summary>
		/// <param name="reader">The data reader to deserialize from.</param>
		public void Deserialize(IDataReader reader)
		{
			RoleId = reader.ReadUInt();
			HideFashion = reader.ReadByte();
			HideSuite = reader.ReadByte();
			HideHelm = reader.ReadByte();
		}

    }
}