// <auto-generated />
// This file was generated from XML metalib definition
// Do not modify this file manually as it will be overwritten
using System;
using System.Collections.Generic;
using Rathalos.Core.Utils.IO;

namespace Rathalos.Core.Protocol.Messages.Csproto
{
    /// <summary>
    /// Secret research lab daily refresh S2C
    /// </summary>
    public class S2CSecretResearchLabDaliyRefreshRsp : CSPkgBody
    {
		public const CS_CMD_ID ProtocolIdConst = CS_CMD_ID.S2C_CMD_SECRET_RESEARCH_LAB_DALIY_REFRESH_RSP;
		/// <summary>
		/// Protocol identifier for this message type
		/// </summary>
		public int ProtocolId => (int)ProtocolIdConst;
		/// <summary>
		/// VIP refresh count
		/// </summary>
		public int VipRefreshCount { get; set; }
		/// <summary>
		/// Refresh count
		/// </summary>
		public int RefreshCount { get; set; }
		/// <summary>
		/// Research count
		/// </summary>
		public int ResearchCount { get; set; }

		/// <summary>
		/// Serializes this instance to the specified data writer.
		/// </summary>
		/// <param name="writer">The data writer to serialize to.</param>
		public void Serialize(IDataWriter writer)
		{
			writer.WriteInt(VipRefreshCount);
			writer.WriteInt(RefreshCount);
			writer.WriteInt(ResearchCount);
		}

		/// <summary>
		/// Deserializes this instance from the specified data reader.
		/// </summary>
		/// <param name="reader">The data reader to deserialize from.</param>
		public void Deserialize(IDataReader reader)
		{
			VipRefreshCount = reader.ReadInt();
			RefreshCount = reader.ReadInt();
			ResearchCount = reader.ReadInt();
		}

    }
}