// <auto-generated />
// This file was generated from XML metalib definition
// Do not modify this file manually as it will be overwritten
using System;
using System.Collections.Generic;
using Rathalos.Core.Utils.IO;

namespace Rathalos.Core.Protocol.Messages.Csproto
{
    /// <summary>
    /// Open recasting attribute request
    /// </summary>
    public class CSItemRebuildFoundOpenReq : CSPkgBody
    {
		public const CS_CMD_ID ProtocolIdConst = CS_CMD_ID.CS_CMD_ITEMREBUILD_FOUND_OPEN_REQ;
		/// <summary>
		/// Protocol identifier for this message type
		/// </summary>
		public int ProtocolId => (int)ProtocolIdConst;
		/// <summary>
		/// Column
		/// </summary>
		public byte EquipColumn { get; set; }
		/// <summary>
		/// Grid
		/// </summary>
		public ushort EquipGrid { get; set; }
		/// <summary>
		/// Attribute index
		/// </summary>
		public byte Attr { get; set; }

		/// <summary>
		/// Serializes this instance to the specified data writer.
		/// </summary>
		/// <param name="writer">The data writer to serialize to.</param>
		public void Serialize(IDataWriter writer)
		{
			writer.WriteByte(EquipColumn);
			writer.WriteUShort(EquipGrid);
			writer.WriteByte(Attr);
		}

		/// <summary>
		/// Deserializes this instance from the specified data reader.
		/// </summary>
		/// <param name="reader">The data reader to deserialize from.</param>
		public void Deserialize(IDataReader reader)
		{
			EquipColumn = reader.ReadByte();
			EquipGrid = reader.ReadUShort();
			Attr = reader.ReadByte();
		}

    }
}