// <auto-generated />
// This file was generated from XML metalib definition
// Do not modify this file manually as it will be overwritten
using System;
using System.Collections.Generic;
using Rathalos.Core.Utils.IO;

namespace Rathalos.Core.Protocol.Messages.Csproto
{
    /// <summary>
    /// Legendary skill pearl - confirm replace correction attribute - response
    /// </summary>
    public class CSLegendPearlConfirmReplaceAbilityRsp : CSPkgBody
    {
		public const CS_CMD_ID ProtocolIdConst = CS_CMD_ID.S2C_CMD_LEGENDPEARL_CONFIRM_REPLACE_ABILITY_RSP;
		/// <summary>
		/// Protocol identifier for this message type
		/// </summary>
		public int ProtocolId => (int)ProtocolIdConst;
		/// <summary>
		/// Error code
		/// </summary>
		public int ErrCode { get; set; }
		/// <summary>
		/// Legendary skill pearl item instance
		/// </summary>
		public ulong ItemID { get; set; }
		/// <summary>
		/// Skill pearl slot column
		/// </summary>
		public byte Column { get; set; }
		/// <summary>
		/// Skill pearl slot grid
		/// </summary>
		public ushort Grid { get; set; }

		/// <summary>
		/// Serializes this instance to the specified data writer.
		/// </summary>
		/// <param name="writer">The data writer to serialize to.</param>
		public void Serialize(IDataWriter writer)
		{
			writer.WriteInt(ErrCode);
			writer.WriteULong(ItemID);
			writer.WriteByte(Column);
			writer.WriteUShort(Grid);
		}

		/// <summary>
		/// Deserializes this instance from the specified data reader.
		/// </summary>
		/// <param name="reader">The data reader to deserialize from.</param>
		public void Deserialize(IDataReader reader)
		{
			ErrCode = reader.ReadInt();
			ItemID = reader.ReadULong();
			Column = reader.ReadByte();
			Grid = reader.ReadUShort();
		}

    }
}