// <auto-generated />
// This file was generated from XML metalib definition
// Do not modify this file manually as it will be overwritten
using System;
using System.Collections.Generic;
using Rathalos.Core.Utils.IO;

namespace Rathalos.Core.Protocol.Messages.Csproto
{
    public class S2CSoulBeastAttrSync : CSPkgBody
    {
		public const CS_CMD_ID ProtocolIdConst = CS_CMD_ID.S2C_CMD_SOULBEAST_ATTR_SYNC_NTF;
		/// <summary>
		/// Protocol identifier for this message type
		/// </summary>
		public int ProtocolId => (int)ProtocolIdConst;
		public uint EntityID { get; set; }
		public uint AttrID { get; set; }
		public short BonusID { get; set; }
		/// <summary>
		/// Variable value
		/// </summary>
		public CSAttrBaseData Data { get; set; }

		/// <summary>
		/// Serializes this instance to the specified data writer.
		/// </summary>
		/// <param name="writer">The data writer to serialize to.</param>
		public void Serialize(IDataWriter writer)
		{
			writer.WriteUInt(EntityID);
			writer.WriteUInt(AttrID);
			writer.WriteShort(BonusID);
			Data?.Serialize(writer);
		}

		/// <summary>
		/// Deserializes this instance from the specified data reader.
		/// </summary>
		/// <param name="reader">The data reader to deserialize from.</param>
		public void Deserialize(IDataReader reader)
		{
			EntityID = reader.ReadUInt();
			AttrID = reader.ReadUInt();
			BonusID = reader.ReadShort();
			Data = new CSAttrBaseData();
			Data.Deserialize(reader);
		}

    }
}