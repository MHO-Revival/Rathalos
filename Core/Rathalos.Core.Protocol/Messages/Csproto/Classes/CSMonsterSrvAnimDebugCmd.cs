// <auto-generated />
// This file was generated from XML metalib definition
// Do not modify this file manually as it will be overwritten
using System;
using System.Collections.Generic;
using Rathalos.Core.Utils.IO;

namespace Rathalos.Core.Protocol.Messages.Csproto
{
    /// <summary>
    /// Monster server animation debug
    /// </summary>
    public class CSMonsterSrvAnimDebugCmd : CSPkgBody
    {
		public const CS_CMD_ID ProtocolIdConst = CS_CMD_ID.CS_CMD_MONSTER_DEBUGSVRANIM;
		/// <summary>
		/// Protocol identifier for this message type
		/// </summary>
		public int ProtocolId => (int)ProtocolIdConst;
		/// <summary>
		/// Monster ID
		/// </summary>
		public uint MonsterID { get; set; }
		/// <summary>
		/// 0: SkeletonPose; 1: PhysicalPart
		/// </summary>
		public byte DebugMode { get; set; }
		/// <summary>
		/// Enable skeleton debug
		/// </summary>
		public byte EnableDebug { get; set; }

		/// <summary>
		/// Serializes this instance to the specified data writer.
		/// </summary>
		/// <param name="writer">The data writer to serialize to.</param>
		public void Serialize(IDataWriter writer)
		{
			writer.WriteUInt(MonsterID);
			writer.WriteByte(DebugMode);
			writer.WriteByte(EnableDebug);
		}

		/// <summary>
		/// Deserializes this instance from the specified data reader.
		/// </summary>
		/// <param name="reader">The data reader to deserialize from.</param>
		public void Deserialize(IDataReader reader)
		{
			MonsterID = reader.ReadUInt();
			DebugMode = reader.ReadByte();
			EnableDebug = reader.ReadByte();
		}

    }
}