// <auto-generated />
// This file was generated from XML metalib definition
// Do not modify this file manually as it will be overwritten
using System;
using System.Collections.Generic;
using Rathalos.Core.Utils.IO;

namespace Rathalos.Core.Protocol.Messages.Csproto
{
    /// <summary>
    /// Get guild member set
    /// </summary>
    public class S2CGetGuilders : CSPkgBody
    {
		public const CS_CMD_ID ProtocolIdConst = CS_CMD_ID.S2C_CMD_GUILD_GETGUILDERS;
		/// <summary>
		/// Protocol identifier for this message type
		/// </summary>
		public ushort ProtocolId => (ushort)ProtocolIdConst;
		/// <summary>
		/// Member set
		/// </summary>
		public int GuildersCount { get; set; }
		/// <summary>
		/// Page set
		/// </summary>
		public int Pages { get; set; }
		/// <summary>
		/// Page
		/// </summary>
		public int Page { get; set; }
		/// <summary>
		/// Member set
		/// </summary>
		public CSGuilders Guilders { get; set; }

		/// <summary>
		/// Serializes this instance to the specified data writer.
		/// </summary>
		/// <param name="writer">The data writer to serialize to.</param>
		public void Serialize(IDataWriter writer)
		{
			writer.WriteInt(GuildersCount);
			writer.WriteInt(Pages);
			writer.WriteInt(Page);
			Guilders?.Serialize(writer);
		}

		/// <summary>
		/// Deserializes this instance from the specified data reader.
		/// </summary>
		/// <param name="reader">The data reader to deserialize from.</param>
		public void Deserialize(IDataReader reader)
		{
			GuildersCount = reader.ReadInt();
			Pages = reader.ReadInt();
			Page = reader.ReadInt();
			Guilders = new CSGuilders();
			Guilders.Deserialize(reader);
		}

    }
}