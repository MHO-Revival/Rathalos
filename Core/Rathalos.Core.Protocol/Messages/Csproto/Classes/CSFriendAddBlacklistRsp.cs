// <auto-generated />
// This file was generated from XML metalib definition
// Do not modify this file manually as it will be overwritten
using System;
using System.Collections.Generic;
using Rathalos.Core.Utils.IO;

namespace Rathalos.Core.Protocol.Messages.Csproto
{
    /// <summary>
    /// Add player to blacklist response
    /// </summary>
    public class CSFriendAddBlacklistRsp : CSPkgBody
    {
		public const CS_CMD_ID ProtocolIdConst = CS_CMD_ID.CS_CMD_FRIEND_ADD_BLACKLIST_RSP;
		/// <summary>
		/// Protocol identifier for this message type
		/// </summary>
		public int ProtocolId => (int)ProtocolIdConst;
		/// <summary>
		/// Target role DBID
		/// </summary>
		public ulong RoleDBID { get; set; }
		/// <summary>
		/// Target role NetID
		/// </summary>
		public int NetID { get; set; }
		/// <summary>
		/// Target role level
		/// </summary>
		public int Level { get; set; }
		/// <summary>
		/// Target role HR level
		/// </summary>
		public int HRLevel { get; set; }
		/// <summary>
		/// Target role name
		/// </summary>
		public string RoleName { get; set; }
		/// <summary>
		/// Online status (0 or 1)
		/// </summary>
		public byte Online { get; set; }

		/// <summary>
		/// Serializes this instance to the specified data writer.
		/// </summary>
		/// <param name="writer">The data writer to serialize to.</param>
		public void Serialize(IDataWriter writer)
		{
			writer.WriteULong(RoleDBID);
			writer.WriteInt(NetID);
			writer.WriteInt(Level);
			writer.WriteInt(HRLevel);
			if (RoleName.Length != CsprotoConstants.CS_MAX_ROLE_NAME)
			{
				throw new InvalidOperationException($"Array length of 'RoleName' should be of length of {CsprotoConstants.CS_MAX_ROLE_NAME} but was {RoleName.Length}.");
			}

			writer.WriteUTF(RoleName);
			writer.WriteByte(Online);
		}

		/// <summary>
		/// Deserializes this instance from the specified data reader.
		/// </summary>
		/// <param name="reader">The data reader to deserialize from.</param>
		public void Deserialize(IDataReader reader)
		{
			RoleDBID = reader.ReadULong();
			NetID = reader.ReadInt();
			Level = reader.ReadInt();
			HRLevel = reader.ReadInt();
			RoleName = reader.ReadUTF();
			Online = reader.ReadByte();
		}

    }
}