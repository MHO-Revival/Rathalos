// <auto-generated />
// This file was generated from XML metalib definition
// Do not modify this file manually as it will be overwritten
using System;
using System.Collections.Generic;
using Rathalos.Core.Utils.IO;

namespace Rathalos.Core.Protocol.Messages.Csproto
{
    public class CSGuildCampOperationRsp : CSPkgBody
    {
		public const CS_CMD_ID ProtocolIdConst = CS_CMD_ID.CS_CMD_GUILDCAMP_OPERATION_RSP;
		/// <summary>
		/// Protocol identifier for this message type
		/// </summary>
		public int ProtocolId => (int)ProtocolIdConst;
		/// <summary>
		/// Error code
		/// </summary>
		public int ErrCode { get; set; }
		/// <summary>
		/// Type
		/// </summary>
		public int OperType { get; set; }
		/// <summary>
		/// Operator LEID
		/// </summary>
		public int OperLogicEntityID { get; set; }
		/// <summary>
		/// Parameter 1
		/// </summary>
		public int Arg1 { get; set; }
		/// <summary>
		/// Parameter 2
		/// </summary>
		public int Arg2 { get; set; }
		/// <summary>
		/// Parameter 3
		/// </summary>
		public int Arg3 { get; set; }
		/// <summary>
		/// Parameter 4
		/// </summary>
		public int Arg4 { get; set; }

		/// <summary>
		/// Serializes this instance to the specified data writer.
		/// </summary>
		/// <param name="writer">The data writer to serialize to.</param>
		public void Serialize(IDataWriter writer)
		{
			writer.WriteInt(ErrCode);
			writer.WriteInt(OperType);
			writer.WriteInt(OperLogicEntityID);
			writer.WriteInt(Arg1);
			writer.WriteInt(Arg2);
			writer.WriteInt(Arg3);
			writer.WriteInt(Arg4);
		}

		/// <summary>
		/// Deserializes this instance from the specified data reader.
		/// </summary>
		/// <param name="reader">The data reader to deserialize from.</param>
		public void Deserialize(IDataReader reader)
		{
			ErrCode = reader.ReadInt();
			OperType = reader.ReadInt();
			OperLogicEntityID = reader.ReadInt();
			Arg1 = reader.ReadInt();
			Arg2 = reader.ReadInt();
			Arg3 = reader.ReadInt();
			Arg4 = reader.ReadInt();
		}

    }
}