// <auto-generated />
// This file was generated from XML metalib definition
// Do not modify this file manually as it will be overwritten
using System;
using System.Collections.Generic;
using Rathalos.Core.Utils.IO;

namespace Rathalos.Core.Protocol.Messages.Csproto
{
    /// <summary>
    /// Server notify script activity variable change
    /// </summary>
    public class S2CScriptActivityVarUpdateNtf : CSPkgBody
    {
		public const CS_CMD_ID ProtocolIdConst = CS_CMD_ID.S2C_CMD_SACTIVITY_VAR_UPDATE_NTF;
		/// <summary>
		/// Protocol identifier for this message type
		/// </summary>
		public int ProtocolId => (int)ProtocolIdConst;
		/// <summary>
		/// Activity ID
		/// </summary>
		public uint ID { get; set; } = 0;
		/// <summary>
		/// Variable index
		/// </summary>
		public uint VarIdx { get; set; } = 0;
		/// <summary>
		/// Variable value
		/// </summary>
		public int VarValue { get; set; } = 0;

		/// <summary>
		/// Serializes this instance to the specified data writer.
		/// </summary>
		/// <param name="writer">The data writer to serialize to.</param>
		public void Serialize(IDataWriter writer)
		{
			writer.WriteUInt(ID);
			writer.WriteUInt(VarIdx);
			writer.WriteInt(VarValue);
		}

		/// <summary>
		/// Deserializes this instance from the specified data reader.
		/// </summary>
		/// <param name="reader">The data reader to deserialize from.</param>
		public void Deserialize(IDataReader reader)
		{
			ID = reader.ReadUInt();
			VarIdx = reader.ReadUInt();
			VarValue = reader.ReadInt();
		}

    }
}