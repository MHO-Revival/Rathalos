// <auto-generated />
// This file was generated from XML metalib definition
// Do not modify this file manually as it will be overwritten
using System;
using System.Collections.Generic;
using Rathalos.Core.Utils.IO;

namespace Rathalos.Core.Protocol.Messages.Csproto
{
    public class CSSurrenderVoteNtf : CSPkgBody
    {
		public const CS_CMD_ID ProtocolIdConst = CS_CMD_ID.CS_CMD_SURRENDER_VOTE_NTF;
		/// <summary>
		/// Protocol identifier for this message type
		/// </summary>
		public int ProtocolId => (int)ProtocolIdConst;
		/// <summary>
		/// Role name
		/// </summary>
		public string Name { get; set; }
		/// <summary>
		/// QQ number
		/// </summary>
		public uint Uin { get; set; }
		/// <summary>
		/// Agree/Disagree
		/// </summary>
		public char Agree { get; set; }
		/// <summary>
		/// Agree vote count
		/// </summary>
		public char AgreeNum { get; set; }
		/// <summary>
		/// Disagree vote count
		/// </summary>
		public char DisagreeNum { get; set; }
		/// <summary>
		/// Total vote count
		/// </summary>
		public char TotalNum { get; set; }

		/// <summary>
		/// Serializes this instance to the specified data writer.
		/// </summary>
		/// <param name="writer">The data writer to serialize to.</param>
		public void Serialize(IDataWriter writer)
		{
			if (Name.Length > (CsprotoConstants.CS_MAX_ROLE_NAME - 1))
			{
				throw new InvalidOperationException($"String length of 'Name' should be of length of {(CsprotoConstants.CS_MAX_ROLE_NAME - 1)} but was {Name.Length}.");
			}

			writer.WriteUTF(Name);
			writer.WriteUInt(Uin);
			writer.WriteChar(Agree);
			writer.WriteChar(AgreeNum);
			writer.WriteChar(DisagreeNum);
			writer.WriteChar(TotalNum);
		}

		/// <summary>
		/// Deserializes this instance from the specified data reader.
		/// </summary>
		/// <param name="reader">The data reader to deserialize from.</param>
		public void Deserialize(IDataReader reader)
		{
			Name = reader.ReadUTF();
			Uin = reader.ReadUInt();
			Agree = reader.ReadChar();
			AgreeNum = reader.ReadChar();
			DisagreeNum = reader.ReadChar();
			TotalNum = reader.ReadChar();
		}

    }
}