...
namespace Rathalos.Core.Protocol.Messages.Csproto
{
    ...
    public class BinCSAny : CSAny
    {
		...
		{
			if (Value.Length > (CsprotoConstants.CS_MAX_ANY_BIN_LEN - 1))
			{
				throw new InvalidOperationException($"String length of 'Value' should be of length of {(CsprotoConstants.CS_MAX_ANY_BIN_LEN - 1)} but was {Value.Length}.");
			}
