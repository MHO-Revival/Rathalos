...
namespace Rathalos.Core.Protocol.Messages.Csproto
{
    ...
    public class StrCSAny : CSAny
    {
		...
		{
			if (Value.Length > (CsprotoConstants.CS_MAX_ANY_STR_LEN - 1))
			{
				throw new InvalidOperationException($"String length of 'Value' should be of length of {(CsprotoConstants.CS_MAX_ANY_STR_LEN - 1)} but was {Value.Length}.");
			}
