// <auto-generated />
// This file was generated from XML metalib definition
// Do not modify this file manually as it will be overwritten
using System;
using System.Collections.Generic;
using Rathalos.Core.Utils.IO;

namespace Rathalos.Core.Protocol.Messages.Csproto
{
    /// <summary>
    /// BT object simple locomotion
    /// </summary>
    public class CSBTObjSimpleLocomotion : CSPkgBody
    {
		public const CS_CMD_ID ProtocolIdConst = CS_CMD_ID.CS_CMD_BTOBJ_SIMPLE_LOCOMOTION;
		/// <summary>
		/// Protocol identifier for this message type
		/// </summary>
		public ushort ProtocolId => (ushort)ProtocolIdConst;
		/// <summary>
		/// NetObjId of BT object to sync
		/// </summary>
		public uint EntityId { get; set; }
		/// <summary>
		/// Current position
		/// </summary>
		public CSVec3 Position { get; set; }
		/// <summary>
		/// Current rotation
		/// </summary>
		public CSQuat Rotation { get; set; }
		/// <summary>
		/// Target position
		/// </summary>
		public CSVec3 TargetPos { get; set; }

		/// <summary>
		/// Serializes this instance to the specified data writer.
		/// </summary>
		/// <param name="writer">The data writer to serialize to.</param>
		public void Serialize(IDataWriter writer)
		{
			writer.WriteUInt(EntityId);
			Position?.Serialize(writer);
			Rotation?.Serialize(writer);
			TargetPos?.Serialize(writer);
		}

		/// <summary>
		/// Deserializes this instance from the specified data reader.
		/// </summary>
		/// <param name="reader">The data reader to deserialize from.</param>
		public void Deserialize(IDataReader reader)
		{
			EntityId = reader.ReadUInt();
			Position = new CSVec3();
			Position.Deserialize(reader);
			Rotation = new CSQuat();
			Rotation.Deserialize(reader);
			TargetPos = new CSVec3();
			TargetPos.Deserialize(reader);
		}

    }
}