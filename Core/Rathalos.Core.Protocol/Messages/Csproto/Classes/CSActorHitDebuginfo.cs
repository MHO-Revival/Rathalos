// <auto-generated />
// This file was generated from XML metalib definition
// Do not modify this file manually as it will be overwritten
using System;
using System.Collections.Generic;
using Rathalos.Core.Utils.IO;

namespace Rathalos.Core.Protocol.Messages.Csproto
{
    /// <summary>
    /// Hit info for statistics
    /// </summary>
    public class CSActorHitDebuginfo : CSPkgBody
    {
		public const CS_CMD_ID ProtocolIdConst = CS_CMD_ID.CS_CMD_BATTLE_ACTOR_HITDEBUGINFO;
		/// <summary>
		/// Protocol identifier for this message type
		/// </summary>
		public int ProtocolId => (int)ProtocolIdConst;
		/// <summary>
		/// Attacker NetObjId
		/// </summary>
		public uint AttackerNetObjId { get; set; }
		/// <summary>
		/// Target NetObjId
		/// </summary>
		public uint TargetNetObjId { get; set; }
		/// <summary>
		/// StateID
		/// </summary>
		public int StateID { get; set; }

		/// <summary>
		/// Serializes this instance to the specified data writer.
		/// </summary>
		/// <param name="writer">The data writer to serialize to.</param>
		public void Serialize(IDataWriter writer)
		{
			writer.WriteUInt(AttackerNetObjId);
			writer.WriteUInt(TargetNetObjId);
			writer.WriteInt(StateID);
		}

		/// <summary>
		/// Deserializes this instance from the specified data reader.
		/// </summary>
		/// <param name="reader">The data reader to deserialize from.</param>
		public void Deserialize(IDataReader reader)
		{
			AttackerNetObjId = reader.ReadUInt();
			TargetNetObjId = reader.ReadUInt();
			StateID = reader.ReadInt();
		}

    }
}