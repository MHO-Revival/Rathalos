// <auto-generated />
// This file was generated from XML metalib definition
// Do not modify this file manually as it will be overwritten
using System;
using System.Collections.Generic;
using Rathalos.Core.Utils.IO;

namespace Rathalos.Core.Protocol.Messages.Csproto
{
    /// <summary>
    /// Player region change request
    /// </summary>
    public class CSPlayerRegionJumpReq : CSPkgBody
    {
		public const CS_CMD_ID ProtocolIdConst = CS_CMD_ID.CS_CMD_PLAYER_REGION_JUMP_REQ;
		/// <summary>
		/// Protocol identifier for this message type
		/// </summary>
		public ushort ProtocolId => (ushort)ProtocolIdConst;
		/// <summary>
		/// Client player current position
		/// </summary>
		public CSVec3 PlayerPos { get; set; }
		/// <summary>
		/// Region change trigger name
		/// </summary>
		public string TriggerName { get; set; }

		/// <summary>
		/// Serializes this instance to the specified data writer.
		/// </summary>
		/// <param name="writer">The data writer to serialize to.</param>
		public void Serialize(IDataWriter writer)
		{
			PlayerPos?.Serialize(writer);
			if (TriggerName.Length != CsprotoConstants.CS_MAX_REGION_JUMP_TRIGGER_NAME)
			{
				throw new InvalidOperationException($"Array length of 'TriggerName' should be of length of {CsprotoConstants.CS_MAX_REGION_JUMP_TRIGGER_NAME} but was {TriggerName.Length}.");
			}

			writer.WriteUTF(TriggerName);
		}

		/// <summary>
		/// Deserializes this instance from the specified data reader.
		/// </summary>
		/// <param name="reader">The data reader to deserialize from.</param>
		public void Deserialize(IDataReader reader)
		{
			PlayerPos = new CSVec3();
			PlayerPos.Deserialize(reader);
			TriggerName = reader.ReadUTF();
		}

    }
}