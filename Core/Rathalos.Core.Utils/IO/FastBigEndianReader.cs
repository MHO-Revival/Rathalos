...
namespace Rathalos.Core.Utils.IO
{
	...
	public unsafe class FastBigEndianReader : IDataReader
	{
		...
		public long Position
		{
			...
			set
			{
				if (_maxPosition > 0 && value > _maxPosition)
		...
		{
			get { return (_maxPosition > 0 ? _maxPosition : _buffer.Length) - Position; }
		}
		...
		public FastBigEndianReader(ReadOnlyMemory<byte> buffer)
		{
			_buffer = buffer;
		}
		...
		{
			fixed (byte* pbyte = &_buffer.Span[(int)Position++])
			{
				return *pbyte;
			}
		}
		...
		public byte[] ReadBytes(int n)
		{
			...
			var dst = new byte[n];
			fixed (byte* pSrc = &_buffer.Span[(int)Position], pDst = dst)
			{
				byte* ps = pSrc;
				byte* pd = pDst;
