...
namespace Rathalos.CLI.CodeGeneration.Templates
{
    public partial class ClassTemplate
    {
        ...
        /// </summary>
        /// <returns>Generated constructors code</returns>
        public string GenerateConstructors()
        {
            ...
            foreach (var constructor in Model.Constructors)
            {
                ...
                if (!string.IsNullOrWhiteSpace(constructor.Description))
                {
                    ...
                    {
                        sb.AppendLine($"\t\t/// <param name=\"{param.Name}\">The {param.Type} value.</param>");
                    }
                }

                // Generate constructor signature
                var parameterList = string.Join(", ", constructor.Parameters.Select(p => $"{p.Type} {p.Name}"));
                sb.AppendLine($"\t\tpublic {Model.Name}({parameterList})");
                sb.AppendLine("\t\t{");
        ...
        public string GeneratePropertyCode(GeneratedProperty property)
        {
            ...
            else
            {
                ...
                {
                    var defaultVal = property.DefaultValue;
                    if (!string.IsNullOrWhiteSpace(defaultVal) && property.RealType == "float")
                    {
                        // Ensure float default values have 'f' suffix
                        if (!defaultVal.EndsWith("f", StringComparison.OrdinalIgnoreCase))
                        {
                            defaultVal += "f";
                        }
                    }
        ...
        private string GenerateWriteCode(GeneratedProperty property)
        {
            ...
            if (property.IsArray)
            {
                ...
                else if (!string.IsNullOrWhiteSpace(maxLength))
                {
                    ...
                    sb.AppendLine("\t\t\t{");
                    sb.AppendLine($"\t\t\t\tthrow new InvalidOperationException($\"Array length of '{property.Name}' should be of length of {{{maxLength}}} but was {{{lengthVar}}}.\");");
                    sb.AppendLine("\t\t\t}");
                    sb.AppendLine();
            ...
            else if (IsPrimitiveType(realType))
            {
                if (realType == "string")
                {
                    var stringMaxSize = property.StringMaxSize;
                ...
                if (property.Type != realType && !_writeMethodMapping.ContainsKey(property.Type))
                {
                    if (property.Type == "bool")
                        sb.AppendLine($"\t\t\twriter.{writeMethod}(({realType})({property.Name} ? 1 : 0));");
                    else
        ...
        private string GenerateReadCode(GeneratedProperty property)
        {
            ...
            else if (IsPrimitiveType(realType))
            {
                ...
                {
                    if (property.Type == "bool")
                        sb.AppendLine($"\t\t\t{property.Name} = (reader.{readMethod}() == 1);");
                    else
        ...
        /// Resolves references to constants ({<see cref="GeneratedCodeModel.TargetName"/>}Constants.X) and properties in the current class or nested custom types.
        /// </summary>
        private string GetArrayMaxLength(GeneratedProperty property)
        {
            ...
            {
                // It's a constant, prefix with {CodeModel.TargetName}Constants class
                return $"{CodeModel.TargetName}Constants.{arraySize}";
            }
